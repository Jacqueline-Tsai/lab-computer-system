...
//   still open.
//
// @author Yun Hsuan Tsai (yunhsuat@andrew.cmu.edu)
//
...
pthread_mutex_t sfs_mutexes = PTHREAD_MUTEX_INITIALIZER;
...
ssize_t sfs_read(int fd, char *buf, size_t len)
{
    ...
    pthread_mutex_lock(&sfs_mutexes);

    sfs_mem_filedesc_t *tFile = openFileDescTable[fd];
    if (tFile == NULL)
    {
        pthread_mutex_unlock(&sfs_mutexes);
        return -EBADF;
    }
...
ssize_t sfs_getpos(int fd)
{
    ...
    // Retrieve the current position from the open file descriptor table
    sfs_mem_filedesc_t *fileDesc = openFileDescTable[fd];
    return (ssize_t)fileDesc->currPos;
}
...
ssize_t sfs_seek(int fd, ssize_t delta)
{
    ...
    // Retrieve the file descriptor's current position and file size
    sfs_mem_filedesc_t *fileDesc = openFileDescTable[fd];
    size_t currentPos = fileDesc->currPos;
    size_t fileSize = fileDesc->fileEntry->diskFile->size;

    // Calculate the new position
    ssize_t newPos = (ssize_t)currentPos + delta;
    ...
    // Ensure the new position does not exceed the file size
    if (newPos > (ssize_t)fileSize)
    {
        newPos = (ssize_t)fileSize;
    }

    // Update the file descriptor's position
    fileDesc->currPos = (size_t)newPos;
...
{
    // Validate file names
    if (strlen(old_name) + 1 > SFS_FILE_NAME_SIZE_LIMIT ||
        strlen(new_name) + 1 > SFS_FILE_NAME_SIZE_LIMIT)
    {
        return -ENAMETOOLONG;
    }

    // Check if a disk image is available
    if (getSFSStatus() < 0)
    {
        return -ENOMEDIUM;
    }
    ...
    // Find the old file
    int oldFileEntry = -1;
    for (int i = 0; (unsigned long)i < FILE_COUNT_LIMIT; i++)
    {
        if (superBlock->files[i].first_block != 0 &&
            strcmp(superBlock->files[i].name, old_name) == 0)
        {
            oldFileEntry = i;
            break;
        }
    ...
    // Find the new file and delete it if it exists
    for (int i = 0; (unsigned long)i < FILE_COUNT_LIMIT; i++)
    {
        if (superBlock->files[i].first_block != 0 &&
            strcmp(superBlock->files[i].name, new_name) == 0)
        {
            sfs_remove(new_name);
            break;
        }
    ...
    // Rename the file
    sfs_dir_entry_t *oldEntry = &superBlock->files[oldFileEntry];
    strncpy(oldEntry->name, new_name, SFS_FILE_NAME_SIZE_LIMIT - 1);
    oldEntry->name[SFS_FILE_NAME_SIZE_LIMIT - 1] =
